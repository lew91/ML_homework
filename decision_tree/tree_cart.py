...
class DecisionTree:
    def __init__(self, classifier=True, max_depth=None, n_feats=None, criterion="entropy", seed=None):
        ...
        Parameters
        ------------
        classifier : bool (defatul: True)
        Whether to treat target values as categorical (Tree) or continuous (False)
        max_depth : int (default: None)
        The depth at which to stop growing the tree. If None, grow the tree
        until all leaves are pure.
        n_feats : int (default: None)
        Specifies the number of features to sample on each split. If None, use
        all features on each split.
        criterion : str (default: 'entropy')
        The error criterion to use when calculating split, When 'classifier' is
        False, valid entries are {'mse'}, when 'classifier' is True, valid entries
        are {'entropy', 'gini'}
    ...
    def fit(self, X, Y):
        ...
        An array of integer labels ranging between [0, n_classes-1] for each
        example in X if 'self.classifier'=True else the set of target values
    ...
    def predict(self, X):
        ...
        The integer class labels predicted for each example in X if classifier
        = True, otherwise the predicted traget values.
        """
    ...
    def predict_class_probs(self, X):
        ...
        ----------
        X : numpy array of shape (N, M)
        The training data of N example, each with M features
    ...
    def _impurity_gain(self, Y, split_thresh, feat_values):
        ...
        IG(splie) = loss(parent) - weighted_avg[loss(left_child), loss(right_child)]
        """
        ...
        # compute the weighted avg. of the loss for each children
        n = len(Y)
    ...
    def _traverse(self, X, node, prob=False):
        if isinstance(node, Leaf):
            ...
            return node.value
        if X[node.feature] <= node.threashold:
