...
def generate_sample(random_state=None):
    ...
    return create_captcha(letter, size=(30, 30)), letters.index(letter)


def generate_network():
    # TODO: set(target), missing index 9, ??? -> 'random_state.choice' missing one feature
    dataset, target = zip(*(generate_sample() for i in range(1000)))
...
def test_prediction(word, shear=0.2, scale=1):
